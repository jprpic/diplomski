...
<template>
    <div>
        ...
        />
        <div
            v-if="searchedItems && searchedItems.length > 0"
        ...
        searchedItems() {
            ...
            return this.items.filter((item) =>
                item.name
                    .toString()
                    .toLowerCase()
                    .startsWith(this.input.toLowerCase())
            );
        },
    },
    methods: {
        updateInput(value) {
            ...
            const item = this.items.find(
                (item) => item.name.toLowerCase() === this.input.toLowerCase()
            );
            if (item) {
        ...
        updateStore(index) {
            this.$store.dispatch(
                `${this.updateStoreFunction}`,
                this.items.find((item) => item.index === index).name
            );
        },
    },
    created() {
