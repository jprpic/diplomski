<template>
    <div class="flex w-2/3 flex-wrap">
        <div class="flex-none items-stretch ml-4 mt-2">
            <BreezeLabel for="contacts" value="Vrsta kontakta" />
            <select
                name="contacts"
                id="contacts"
                required
...
export default {
    ...
    computed: {
        contact() {
            const contactClone = JSON.parse(
                JSON.stringify(this.$store.getters.cv.contacts[this.index])
            );
        ...
        contactInfo() {
            const contact = this.availableContacts.find(
                (contact) => contact.id === parseInt(this.contact.contact_id)
            );
            if (contact) {
                if (contact.name === "Email") {
                    return { label: "E-mail adresa", type: "email" };
                } else if (contact.name === "Phone") {
                    return { label: "Telefonski broj", type: "text" };
                } else {
                    return { label: "URL", type: "url" };
                }
            }
            return { label: "Vrijednost", type: "text" };
        },
        availableContacts() {
