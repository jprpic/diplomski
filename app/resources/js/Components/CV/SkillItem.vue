<template>
    <div class="flex ml-2 pb-2">
        <div class="grow mt-2 mx-2 flex-col">
            <div>
                <BreezeLabel for="skill_name" value="Ime" />
                <BreezeInput
                    type="text"
        ...
        <div class="flex-none items-stretch mx-2 mt-2">
            <BreezeLabel for="level" value="Razina znanja" />
...
export default {
    ...
    computed: {
        skillName() {
            if (this.skill.skill_id) {
                return this.availableSkills.find(
                    (skill) => skill.id === this.skill.skill_id
                ).name;
            }
            return this.name;
        },
        skill() {
            const skillClone = JSON.parse(
                JSON.stringify(this.$store.getters.cv.skills[this.index])
            );
        ...
        availableSkills() {
            return usePage().props.value.availableSkills.sort((a, b) =>
