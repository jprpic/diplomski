<template>
    <div class="px-4 py-2 border border-gray-200 rounded-md shadow-md bg-gray-50">
        ...
        <div v-if="searchedSkills.length" class="mt-2 flex w-full justify-center">
            <div class="text-md font-medium w-1/3 rounded-lg bg-gradient-to-r from-lime-400/50 flex flex-col text-center py-2">
                <div v-for="skill in foundSkills">
                    <span class="text-lime-900"> {{ skill.name }} ({{ skill.proficiency }}) </span>
                </div>
            </div>
            <div class="text-md font-medium w-1/3 rounded-lg bg-gradient-to-l from-red-400/50 flex flex-col text-center py-2">
...
export default {
    ...
    computed:{
        ...
        foundSkills(){
            ...
            const userSkills = this.target.skill_proficiencies.filter( el => this.searchedSkills.includes(el['skill_id']));
            const userSkillIDs = userSkills.map(el => el['skill_id']);
            const skills = this.availableSkills.filter(el => userSkillIDs.includes(el.id));
        ...
        missingSkills(){
            ...
            const userSkills = this.target.skill_proficiencies.filter( el => this.searchedSkills.includes(el['skill_id']));
            const userSkillIDs = userSkills.map(el => el['skill_id']);
            const missingSkillIDs = this.searchedSkills.filter( el => !userSkillIDs.includes(el));
            return this.availableSkills.filter(el => missingSkillIDs.includes(el.id))
