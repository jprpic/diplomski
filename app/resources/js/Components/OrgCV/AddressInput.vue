...
<template>
    <div class=" flex w-full flex-wrap w-full">
        <div class="grow-0">
            ...
            <BreezeInput id="address" type="text" class="mt-1 block" :value="orgCv.street" @input="updateAddressStreet($event.target.value)" required  />
        </div>
...
export default {
    ...
    methods:{
        updateAddressStreet(street){
            this.$store.dispatch('updateOrgStreet', street)
        },
        updatePostcode(code){
            this.$store.dispatch('updateOrgPostcode', code)
        },
        fillAddress(){
    ...
    mounted() {
        if(this.orgCv.postcode){
            this.code = this.orgCv.postcode.toString();
            this.fillAddress();
        }
    }
}
